...
class SimpleImage:
    ...
    def not_pass(self):
        ...
        if self.constants["crop"]:
            ...
            while i < len(self.violated_circles):
                center = (
                    self.violated_circles[i][0][0] + (self.constants["boarder"] // 2),
                    self.violated_circles[i][0][1] + (self.constants["boarder"] // 2),
                )
                self.violated_circles[i][0] = center
                i += 1
            self.out_image = area_utils.sum_images(
                self.out_image, self.image_data_backup, self.constants["boarder"]
            )
            self.out_image = image_utils.add_boarder(
                self.out_image, self.constants["boarder"]
            )
        ...
        self.out_image = image_utils.color_holes2(
            self.violated_circles, self.out_image, float(self.constants["scale"])
        )
        ...
        image_utils.save_out_image(
            ...
            + folder
            + "/"
            + os.path.basename(os.path.normpath(self.image_dic["img_path"])),
        )
    ...
    def is_pass(self, regions):
        ...
        large_reg = list(
            filter(lambda reg: reg["area"] * scale_area > min_area, regions)
        )
    ...
    def get_regions(self):
        ...
        regions = list(
            filter(
                lambda reg: reg["area"] * scale_area > self.constants["min_ignore"],
                regions,
            )
        )
    ...
    def prep_image(self):
        ...
        self.image_data = resize(
            self.image_data, dsize=(800, 600), interpolation=INTER_AREA
        )
        if self.constants["crop"]:
            self.image_data = image_utils.get_crop_image(
                self.image_data, self.constants["boarder"]
            )
